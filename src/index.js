...
/* global ace */
// Ace editor setting
const editor = ace.edit('editor') // Set editor to id="editor"
...
// ace editor
let beforeUrl = 'https://docs.freebsd.org/en/books/faq/' // Default url
...
configFile.onreadystatechange = function githubApiGetAdoc() {
	if (configFile.readyState === 4) {
		// 4 means request has finished and has a response
		if (configFile.status === 200 || configFile.status === 0) {
			configText = configFile.responseText // Get text from config.json
			configTextJson = JSON.parse(configText) // Change configText to a JSON object
...
function githubApiGet() {
	...
	const githubUrl =
		...
		beforeUrl.split('#')[0].substring(24) +
		'_index.adoc'
	...
	request.onreadystatechange = function () {
		if (request.readyState === 4) {
			...
			if (request.status === 200) {
				...
				editor.session.insert(editor.getCursorPosition(), adocContent) // Insert .adoc content that GitHub API returned
				// into the left editor session
				window.origin_content = adocContent // Use global window to store content
				fileTitle.innerHTML = beforeUrl // Set adoc file title in the left section
...
function crossrefHandler(htmlContent) {
	...
	for (let i = 0; i < aTags.length; i++) {
		...
		if (href.startsWith('../../')) {
			...
			const generatedString = '../'.repeat(matches.length) // Replace all ../ str
...
window.generateHtml = function generateHtml() {
	asciidoctorSet()
	let editorContent = editor.getValue() // Editor content
	window.editorContent = editorContent // Use global window object to store current content
